...
const size_t TARGET_SIZE_MB = 2;
const size_t NUM_FILES = 8;
const size_t WORDS_PER_CHUNK = 500;
const size_t TARGET_SIZE_BYTES = TARGET_SIZE_MB * 1024 * 1024;
...
int main() {
    ...
    std::cout << "Generating " << NUM_FILES << " files of " << TARGET_SIZE_MB << "MB each...\n";

    auto time_start = std::chrono::high_resolution_clock::now();

    for (size_t i = 1; i <= NUM_FILES; ++i) {
        std::string filename = "benchmark/" + std::to_string(i) + ".txt";
  ...
  auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(time_end - time_start).count();
    ...
    std::cout << "All benchmark files generated in " << elapsed << " ms.\n";
    return 0;
}
